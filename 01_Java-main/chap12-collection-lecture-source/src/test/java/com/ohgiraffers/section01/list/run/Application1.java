...
public class Application1 {
    public static void main(String[] args) {
        ...
        // stirngList.add(123);
        stirngList.add("apple");
        stirngList.add("banana");
        stirngList.add("orange");
        stirngList.add("mango");
        stirngList.add("grape");
        ...
        * Collection 클래스 사용 -> Collection 사용되는 기능을 static 메서드로 구현한 클래스
        * */
        Collections.sort(stirngList);
        Collections.reverse(stirngList);
        System.out.println("stirngList = " + stirngList);

        /*
        z* ArrayList에는 역순으로 정렬하는 기능이 제공되지 않는다.
        * LinkedList decendingItertor() 메소드를 사용하면
        * 내림차순으로 정렬된 Iterator 타입의 목록으로 변환된다.
        * */
        /*
        * Iterator
        * Collection 인터페이스의 iterator() 메소드를 이용해서 인스턴스를 생성 할 수 있다.
        * 컬렉션에서 값을 읽어오는 방식을 통일하게 제공하기 위해서 사용
        *
        * hashNext() : 다음 요소를 가지고 있는 경우 true, 더이상 요소가 없는경우 false 반환
        * next() : 다음요소를 반환
        * */

        stirngList = new LinkedList<>(stirngList);

        Iterator<String> dIter = ((LinkedList<String>)stirngList).descendingIterator();

...
//    private final int nonStaticNum;
    ...
    public FinalkeywordTest(String NON_STATIC) {
        this.NON_STATIC_NAME = NON_STATIC;
    }
    ...
    * final 키워드 사용시 초기화를 하지 않으면 에라가 발생한다.
    * */

    /*
    * 생성자를 이용한 초기화는 불가능
    * 생성자는 인스턴스가 생성되는 시점에 호출 되기 때문에 그 전에는 초기화가 일어나지 못한다.
    * 하지만 static은 프로그램이 start 될 때 할당되기
    * 때문에 초기화가 되지 않은 상태로 선언된 것과 동일하여
